...
- `article` template now supports a `version` front-matter field that surfaces in `\maketitle` underneath the date in a `\small` style. The value is free-form (`version: "Consolidation du draft de janvier 2026"`) or the literal sentinel `version: git`, in which case the template runs `git describe --tags --dirty` against the document's repository (with a 7-character commit-hash fallback, suffixed `-dirty` when the worktree has uncommitted changes) and emits a warning if no git metadata is reachable. The resolver lives in a new `texsmith.core.git_version` module — `format_version()`, `git_describe()`, `resolve_git_root()`, `reset_cache()` — so other templates can opt in. Tag names containing LaTeX-special characters (`_`, `&`, `%`, `#`, …) are escaped by the article template before being injected, so a tag like `v1.0_alpha` renders correctly.
