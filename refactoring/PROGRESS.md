...
| P6 — Parité nœuds Typst | Typst Parity | 🟢 terminé | ✅ | **Aussi : bump `mitex` 0.2.4→0.2.6** (3 sites : `writers/typst/document.py`, `templates/{article,book}/template/template.typ`) — `mitex:0.2.4` est incompatible avec `typst>=0.15` (épinglé dans `pyproject`) : compile échoue `unknown variable: kai` sur tout `#mi/#mitex`. 0.2.6 OK → le paper `cheese` (math inline+display) **compile en PDF** sous typst 0.15. Golden `typst-{paper,article,book}` re-capturés (seule la ligne d'import change). `examples/paper/Makefile` : `make` produit **les deux** PDF (LaTeX `build/<engine>/` + Typst `build/typst/`) + targets `latex`/`typst`. — Comble les 3 derniers écarts Typst↔LaTeX. **Tables riches** (yaml/`data-ts`) : `writer.py::_rich_table_call` reconstruit un `#table` natif depuis `node.model` (regen HTML canonique `render_table_html` + injection des `cells` rendus, comme le chemin yaml LaTeX) → `columns` (largeurs `X`→`1fr`, `NN%`→relatif), `align` par colonne, `table.header(...)`, `table.cell(colspan/rowspan/align)`, `table.hline()` (séparateurs labellisés → cellule pleine largeur `_label_`), footer séparé par une `hline`. **MarginNote** → `#footnote` (dégradation, pas de primitive marge sans package). **ProgressBar** → piste `#box` arrondie + box rempli `width: <pct>` (built-in, sans package). Writer Typst = **parité IR-nœud complète avec LaTeX** (0 nœud sans émetteur). Validé : `examples/tables/tables.md` (groupes, multirow/multicolumn, séparateurs, footer, colonne `X`) + marginnote/progressbar **compilent en PDF** (`typst 0.15.0`) et rendu visuel correct. Nouveau golden `typst-tables` déterministe. `pytest` **848 passed**, golden `diff --skip-docker` **31 capturés / 0 fail**, ruff clean. `ir/` et `readers/` **NON modifiés**. |
