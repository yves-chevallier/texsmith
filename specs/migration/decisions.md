...
| X2 | `---` (`HorizontalRule`): `\clearpage` in the writer or a `\tsdivider` contract? | **`\tsdivider`** provided by `ts-typesetting` (default `\clearpage`), from the first Rust line. The parity allow-list carries a `rewrite` entry. | One migration, not two; a paged template restyles it without touching the writer. |
...
| X5 | Is a Python `glossary` pass needed? | **Verify first, no pass by default.** tmark emits `Abbr` from `press.declare.acronyms` and `*[X]:`; TeXSmith's `glossary:` front matter (`style`, `groups`, `entries`) is `any` in `extra` and is consumed by the `ts-glossary` fragment, not by a pass. A pass is added only if `examples/glossary` and `examples/abbr` fail on the new path. | YAGNI; the fragment already generates the entries from the document. |
