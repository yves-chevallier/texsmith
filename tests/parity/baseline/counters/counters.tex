...
matter under \texttt{counters:}, so TeXSmith allocates the numbers and both backends ---
\LaTeX{} and Typst --- print the same ones.

The requirement series starts at \texttt{100} because it continues the numbering of
the system specification; the finding series starts at \texttt{1}, the default. The
...
\section{Requirements under review}\label{requirements-under-review}

The four requirements below were selected for this review. The identifier in
the first column is a definition marker, \texttt{\#\{prefix:key\}}: it prints the
...
\midrule
\leavevmode\phantomsection\label{fw:watchdog}FW-01 & Blocking & \texttt{hal/watchdog.c} & The watchdog is fed from the I2C completion handler. \\
\leavevmode\phantomsection\label{fw:ota-brick}FW-02 & Blocking & \texttt{ota/apply.c} & A power cut during the swap leaves no bootable image. \\
\leavevmode\phantomsection\label{fw:key-entropy}FW-03 & Major & \texttt{crypto/session.c} & The session key is seeded from the boot counter. \\

\bottomrule
\end{tabularx}
\end{center}

\subsection{Watchdog fed from an interrupt handler}\label{watchdog-fed-from-an-interrupt-handler}
...
\subsection{Over-the-air update leaves no bootable image}\label{over-the-air-update-leaves-no-bootable-image}
...
\subsection{Session key seeded from the boot counter}\label{session-key-seeded-from-the-boot-counter}
...
\subsection{Log buffer wiped on reset}\label{fw:log-wrap}

The heading above carries a \texttt{\{\#prefix:key\}} attribute, a \emph{silent} definition:
the number appears nowhere in the title, yet the finding is allocated one and
can be referenced like any other. The log ring buffer lives in a \texttt{.bss}
...
\subsection{Additional observation}\label{additional-observation}
...
A reference prints the number and nothing else --- \texttt{FW-\allowbreak{}01}, not \texttt{Finding FW-\allowbreak{}01}.
The surrounding noun is written by the author, exactly as for a section
cross-reference; the \texttt{name:} field of the counter is only used in TeXSmith's
diagnostics.

\section{A note on undeclared prefixes}\label{a-note-on-undeclared-prefixes}
...
\#\{sensor:temp\} for the reading. Neither \texttt{node} nor \texttt{sensor} is a declared
counter, so both markers survive into the PDF unchanged --- Ruby and CoffeeScript
interpolations are never mangled, and no warning is emitted. Only \texttt{fw} and
\texttt{req} are declared here, and only those are substituted.
