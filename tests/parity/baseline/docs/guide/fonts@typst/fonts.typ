...
Use the render command with `–fonts-info` to display the scripts and fonts that were detected during a build. The flag works for both direct #ts-logo("LaTeX") output and full template renders:
...
- the script name and the generated #ts-logo("LaTeX") commands (`\text<slug>` / `\<slug>font`);
