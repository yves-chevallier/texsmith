...
`title` is not strictly required. If omitted, TeXSmith falls back to the first heading in the document, on the assumption that you probably meant _that_ to be the title. To opt out of this fallback (and render with no title at all), set `title: null` explicitly:
