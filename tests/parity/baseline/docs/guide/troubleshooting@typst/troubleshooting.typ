#set document(
title: "Troubleshooting LaTeX Builds",
)
#set page(
paper: "a4",
margin: 2.5cm,
...
*Fix:* Ensure your #ts-logo("TeX") Live/MacTeX/MiKTeX distribution added `latexmk` to `PATH`. On macOS, `eval "$(/usr/libexec/path_helper -s)"` after installing MacTeX. On Windows, open the “#ts-logo("LaTeX") apps” PowerShell and run TeXSmith from there.
...
+ Re-run with `–debug –classic-output -vv` to capture both TeXSmith diagnostics and raw `latexmk` logs.
+ Attach the failing `.log` plus your template manifest when filing an issue.
+ Mention your #ts-logo("TeX") distribution and operating system so maintainers can reproduce the environment.
