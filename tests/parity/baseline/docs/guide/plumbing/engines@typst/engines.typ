...
The original TeX engine, created by Donald Knuth, is the minimalist mathematical core of the entire ecosystem. It’s deterministic, stable to the point of obsession, and designed so its output will match _forever_. It handles typesetting with surgical precision but offers no frills—no Unicode, no PDF output, and no modern scripting hooks. Pure, legendary, and a little bit stubborn.
...
+ Tectonic is so smooth, it downloads packages automatically, making it great for newcomers. No need to install a heavy TeX distribution.
+ LuaLaTeX is the only engine that supports both protrusion and font expansion (microtypography) along with Lua scripting, which yields smoother PDF output.
+ XeLaTeX has similar results to Tectonic but allows `--shell-escape` for minted code highlighting.
