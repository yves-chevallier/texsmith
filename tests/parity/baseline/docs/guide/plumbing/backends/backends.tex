...
\end{Verbatim}
\end{tscode}
Both backends are fed by the \textbf{same} IR, the same passes and the same single
...
fidelity yet.
\end{tscallout}
\href{https://typst.app}{Typst} is a modern, Rust-based typesetting system. The body
comes from tmark's Typst writer; \tscodeinline{texsmith.writers.typst} wraps it into a
compilable \tscodeinline{.typ} source. It is a lean, Typst-native path and intentionally
covers a \textbf{subset} of the \LaTeX{} backend (see \hyperref[scope-and-limitations]{Scope}).
\begin{tscode}[lang=bash, engine=pygments]
\begin{Verbatim}[commandchars=\\\{\},breaklines, breakanywhere, commandchars=\\\{\}]
...
\subsection{What the \tscodeinline{.typ} carries}
The Typst writer calls one \tscodeinline{\#ts-\allowbreak{}…} function per contract, the mirror of the
\LaTeX{} contract macros --- \tscodeinline{\#ts-\allowbreak{}callout}, \tscodeinline{\#ts-\allowbreak{}code}, \tscodeinline{\#ts-\allowbreak{}keys}, \tscodeinline{\#ts-\allowbreak{}gls}, and
so on. They are defined in \tscodeinline{src/texsmith/templates/common/texsmith.typ}, and
that library is \textbf{inlined ahead of the body} in the generated \tscodeinline{.typ}, so the
...
after \tscodeinline{\textbackslash{}VAR\{extra\_packages\}}. See Contract macros.

Math is the one external dependency, and it is pinned: a document with math
opens with
\begin{tscode}[lang=typst, engine=pygments]
\begin{Verbatim}[commandchars=\\\{\},breaklines, breakanywhere, commandchars=\\\{\}]
\PY{k+kn}{\PYZsh{}import}\PY{+w}{ }\PY{l+s+s2}{\PYZdq{}@preview/mitex:0.2.7\PYZdq{}}\PY{p}{:}\PY{+w}{ }\PY{n+nv}{mi}\PY{p}{,}\PY{+w}{ }\PY{n+nv}{mitex}
\end{Verbatim}
\end{tscode}
...
\tscodeinline{-\allowbreak{}-\allowbreak{}format typst} cannot be combined with \tscodeinline{-\allowbreak{}-\allowbreak{}template-\allowbreak{}info} or
...
\item The \tscodeinline{ts-\allowbreak{}*} fragments themselves are \LaTeX{} packages and are not loaded on the
Typst path: the \tscodeinline{\#ts-\allowbreak{}…} functions of \tscodeinline{texsmith.typ} stand in for them, so a
fragment's \LaTeX{}-only configuration (the glossary and index engines,
\tscodeinline{ts-\allowbreak{}geometry}, \tscodeinline{ts-\allowbreak{}frame}) has no effect there. Acronyms still reach the
backmatter, and the \tscodeinline{-\allowbreak{}-\allowbreak{}numbering tmark} mode makes the two backends print the
same numbers.
\end{itemize}
