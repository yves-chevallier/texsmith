...
\section{LuaTeX}\label{luatex}

LuaTeX embeds a full Lua interpreter into the engine, effectively giving TeX a programmable runtime. This allows deep customization, dynamic content generation, and powerful extensions like \texttt{luaotfload} and \texttt{luametalatex}. It's the most flexible and hackable TeX engine, almost a TeX/Lua hybrid organism.

\section{Tectonic}\label{tectonic}
...
\section{Omega ($\Omega$) / Aleph ($\aleph$)}\label{omega-o-aleph}
...
\section{pTeX / upTeX}\label{ptex-uptex}
...
\section{Which to prefer?}\label{which-to-prefer}

That a debate as old as TeX itself still rages on is a testament to its complexity and versatility. For most users, \textbf{pdfTeX} or \textbf{LuaTeX} (with \LaTeX{} macros) will cover nearly all needs. Some facts:
...
\item LuaLaTeX is the only engine that supports both protrusion and font expansion (microtypography) along with Lua scripting, which yields smoother PDF output.
\item XeLaTeX has similar results to Tectonic but allows \texttt{-\allowbreak{}-\allowbreak{}shell-\allowbreak{}escape} for minted code highlighting.
