...
\section{Versioning of \TeX{}---Why $\pi$?}
...
The name \enquote{\TeX{}} comes from the Greek root \tsscript{greek}{τεχ}- (ték\tsscript{diacritics}{\textsuperscript{h}}nē) meaning \enquote{art, craft, technique}---fitting for a typesetting system deeply rooted in the art of typesetting. Because the \enquote{X} in \TeX{} is really the Greek letter chi (\tsscript{greek}{Χ}), the \enquote{proper} pronunciation is something like \enquote{tekh} (with a voiceless velar fricative---like the \enquote{ch} in German or Scottish \enquote{loch}).
