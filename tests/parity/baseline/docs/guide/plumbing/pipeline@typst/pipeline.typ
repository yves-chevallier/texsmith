...
+ *Prime context, fragments, and attributes*
...
+ *Render HTML fragments through the IR*
     Each slot fragment is rendered through `LaTeXRenderer`, a thin orchestrator over a single read→write path: the `HtmlReader` (`texsmith.readers.html`) lowers the HTML into a typed intermediate representation (`texsmith.ir`), and the `LaTeXWriter` (`texsmith.writers.latex`) emits LaTeX from that IR (reusing the Jinja partials from step 6 via the `LaTeXFormatter`). Reader lowerings register with `@reads` and return IR nodes; writer emitters register with `@writes` and dispatch by node type — a node without an emitter raises a clear error. Fallback converters are registered when external tools are unavailable. Runtime data (base\_level, numbered flag, drop\_title, bibliography map, partial providers, language, diagram backend) flows through the writer state, and the `DocumentState` accumulates headings, citations, index terms, script usage/fallback font summaries, glossaries, snippets, callouts, and asset references so later stages can emit the right packages and backmatter. The Typst backend (`--format typst`) reuses the same reader and IR, swapping in the `TypstWriter` (see #link("backends.md")[Output backends]).
+ *Fonts and script matching*
...
+ *Bibliography and index resolution*
