...
+ Drop any sections routed to slots (e.g. `abstract`) before aligning the remaining content for that slot.
+ If promotion is on and the first heading is uniquely the highest, it becomes metadata and is ignored for offsets.
+ Look at the shallowest heading that remains:
- `<h1>` #ts-script("symbols")[→ ]offset `0`
- `<h2>` #ts-script("symbols")[→ ]offset `-1`
- `<h3>` #ts-script("symbols")[→ ]offset `-2`
- No headings #ts-script("symbols")[→ ]offset `0`
+ The effective base level for a fragment is:
  ```
  template slot base + document base_level + fragment offset
  ```
+ The rendered #ts-logo("LaTeX") level is `html_level + effective_base - 1`.
...
- Metadata title present; headings start at `##`: fragment offset `-1` + template base `1` #ts-script("symbols")[→ ]first heading renders as `\section`, nested `###` as `\subsection`.
