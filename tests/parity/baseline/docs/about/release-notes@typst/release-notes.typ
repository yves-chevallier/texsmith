...
Use this page to see what changed in each TeXSmith release and which #ts-logo("LaTeX") prerequisites (#ts-logo("TeX") Live year, `tlmgr` packages, shell-escape requirements) the bundled templates expect. Update your automation and CI images accordingly before bumping versions.
...
[`0.4.0`], [Fragment manifest/ABC (attributes, partials, slot validation), explicit partial precedence (template \> fragment \> core), template discovery order (built-ins #ts-script("symbols")[→ ]packages #ts-script("symbols")[→ ]local #ts-script("symbols")[→ ]`~/.texsmith/templates`), improved `–template-info` output (slots, fragments, attribute columns).], [Update custom fragments to `fragment.toml` + `partials`/`required_partials`; remove legacy template attributes (cover/twocolumn/backmatter/emoji) and use fragment-provided options instead. Use `texsmith templates` to confirm discovery.],
)
...
The command lists #ts-logo("TeX") Live requirements, shell-escape expectations, slot definitions, and declared assets.

= Upgrade checklist

+ Run `uv run mkdocs build` to confirm docs compile without warnings on the new release.
+ Update your #ts-logo("TeX") Live image with any new packages referenced in the table above or in `template-info`.
+ Rebuild the examples (`docs/examples/index.md`) and confirm smoke tests still pass.
+ Capture notable migrations (CLI flags, metadata schema changes) in this page and link PRs or release notes for future reference.
