...
\thispagestyle{plain}
\section{Complex Tables}\label{complex-tables}
...
\end{itemize}
\subsection{Extended Markdown Table Syntax}\label{extended-markdown-table-syntax}

Leverage Pymdown's table extension to add more metadata directly in Markdown. For example:
The \texttt{texsmith.spantable} extension lets us span cells in standard Markdown tables.

The \texttt{>>>} syntax will span cells horizontally, the \texttt{vvv} syntax will span cells vertically.
...
\end{Verbatim}
\end{code}
\subsection{Cmidrule example}\label{cmidrule-example}
...
\end{Verbatim}
\end{code}
\subsection{Align to decimal points}\label{align-to-decimal-points}

Find a syntax to align numbers to dot. \texttt{lS@\{\}}

\subsection{Raw table syntax}\label{raw-table-syntax}

Superfences do not work directly with tables, so define a \texttt{table} fence that accepts YAML options:
...
\section{Shrink-to-fit vs Auto-expand}\label{shrink-to-fit-vs-auto-expand}
...
\end{Verbatim}
\end{code}
We could use a tester to have a rough idea if the table will fit or not, and decide to use \texttt{tabularx} or \texttt{ltablex} accordingly.
