...
\section{Standalone Plugins}\label{standalone-plugins}
...
\end{Verbatim}
\end{code}
This is inserted into the \LaTeX{} output using the \texttt{epigraph} package:
...
\end{Verbatim}
\end{code}
\subsection{SvgBob}\label{svgbob}

This can be a good example of a standalone TeXSmith plugin that allows rendering ASCII art diagrams using Svgbob.

\href{https://github.com/ivanceras/svgbob}{Svgbob} lets you sketch diagrams using ASCII art. Save the source with a \texttt{.bob} extension (or keep it inline) and link to it like any other image:
...
\end{Verbatim}
\end{code}
It is installed by default into \texttt{svgbob\_cli} or \texttt{\textasciitilde{}/.cargo/bin/svgbob\_cli}. We want to check both, warn if the binary is missing, and allow users to override the path via configuration.
...
\subsection{CircuitTikZ}\label{circuittikz}
...
\end{Verbatim}
\end{code}
Raw blocks bypass the HTML output but remain in the \LaTeX{} build. To keep the TikZ code in a separate file, include it via \texttt{\textbackslash{}input\{\}} inside a raw fence and store the \texttt{.tex} asset alongside the Markdown.

\section{Module Design Principles}\label{module-design-principles}
...
\end{itemize}
\section{Visual Tweaks}\label{visual-tweaks}
...
\item{} \texttt{\{\textasciitilde{}\textasciitilde{}deleted text\textasciitilde{}\textasciitilde{}\}} should drop the curly braces, which currently leak into the output.

\end{itemize}
\section{Issues}\label{issues}

\subsection{Markdown Package Issues}\label{markdown-package-issues}

\texttt{mkdocstrings} autorefs define heading anchors via \texttt{[]()\{\#\}}, which triggers Markdown lint violations. Find a syntax or lint configuration that avoids false positives.
