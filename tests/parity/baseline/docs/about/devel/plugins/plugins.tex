...
\end{Verbatim}
\end{tscode}
\tscodeinline{latex raw} is the canonical spelling of a raw fence; \tscodeinline{/// latex … ///} is read
as deprecated sugar for it and \tscodeinline{tmark lint -\allowbreak{}-\allowbreak{}fix} rewrites it. Raw blocks bypass
the HTML output but remain in the \LaTeX{} build. To keep the TikZ code in a separate file, include it via \tscodeinline{\textbackslash{}input\{\}} inside a raw fence and store the \tscodeinline{.tex} asset alongside the Markdown.
...
\item Critic markup is not lowered by the parser yet: \tscodeinline{\{++inserted++\}},
\tscodeinline{\{-\allowbreak{}-\allowbreak{}deleted-\allowbreak{}-\allowbreak{}\}}, \tscodeinline{\{\textasciitilde{}\textasciitilde{}old\textasciitilde{}\textasciitilde{}new\textasciitilde{}\textasciitilde{}\}} and \tscodeinline{\{>>comment<<\}} stay literal text and
raise \tscodeinline{compat-\allowbreak{}unsupported}, so the curly braces no longer leak silently ---
they are reported. The \tscodeinline{ts-\allowbreak{}critic} fragment already defines \tscodeinline{\textbackslash{}tsins},
\tscodeinline{\textbackslash{}tsdel}, \tscodeinline{\textbackslash{}tssubst} and \tscodeinline{\textbackslash{}tscomment} over \tscodeinline{ulem} and \tscodeinline{xcolor}; restyle
...
\tscodeinline{mkdocstrings} autorefs define heading anchors via \tscodeinline{[]()\{\#id\}}, which triggers Markdown lint violations. TMark reads the pair as the anchor it was meant to be (it used to produce an empty \tscodeinline{\textbackslash{}url\{\}} and lose the \tscodeinline{\textbackslash{}label}), reports it as \tscodeinline{deprecated} and offers \tscodeinline{[]\{\#id\}} as the fix --- which is the spelling to prefer once autorefs indexes it.
\end{document}
