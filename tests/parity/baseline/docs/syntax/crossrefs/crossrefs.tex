...
document citing \texttt{FW-\allowbreak{}10} has no way to know what \texttt{FW-\allowbreak{}10} is, and --- worse --- no way
to notice when a renumbering turns it into \texttt{FW-\allowbreak{}12}. Hard-coded numbers in a
sister document break silently, which is exactly the maintenance
\hyperref[snippet:<HASH>]{custom counters} remove \emph{inside} a document.

TeXSmith closes the loop the way DocBook's \emph{target database} and Sphinx's
\texttt{objects.inv} do: every conversion publishes a small JSON \textbf{inventory} next to
...
\end{displayquote}
\section{Identifying a document}\label{identifying-a-document}

\texttt{id} is a free label --- a contract number, a report reference, whatever your
organisation numbers documents with (\texttt{document-\allowbreak{}id} is accepted as an alias). When the target document declares one,
a citation concatenates it with the item's label: \texttt{RHE-\allowbreak{}423} + \texttt{FW-\allowbreak{}10} \(\rightarrow\)
\textbf{\texttt{RHE-\allowbreak{}423-\allowbreak{}FW-\allowbreak{}10}}. That is what makes the reference unambiguous outside the
document that defines it.

It is optional. Without it, a citation falls back to naming the document by its
title --- \texttt{FW-\allowbreak{}10 (Revue firmware, p. 14)} --- and without a title either, to the
bare label.

\section{The inventory}\label{the-inventory}

Each conversion writes \texttt{<document>.refs.json} beside its \texttt{.tex}:
...
\texttt{-\allowbreak{}o dir}, beside the PDF for \texttt{-\allowbreak{}o report.pdf}. Its \texttt{document.source} is stored
...
\texttt{.aux} afterwards --- the same source the \texttt{xr} package reads. A conversion without
\texttt{-\allowbreak{}-\allowbreak{}build} therefore publishes an inventory without pages, and citations simply
omit the page.

\begin{callout}[callout tip]{Commit the inventory}
It is a build artifact, but committing it makes renumbering \emph{diffable}:
\texttt{git diff} shows \texttt{FW-\allowbreak{}04 \(\rightarrow\) FW-\allowbreak{}10} before you publish. For a contractual
document, that diff is the review gate that catches a renumbering before it
reaches a reader.
\end{callout}
\section{Declaring and citing}\label{declaring-and-citing}
...
of the reference: \texttt{@fwrev:fw:pas-\allowbreak{}de-\allowbreak{}temps}.

Resolution is \textbf{explicit}: an alias never falls back to a local counter, and a
local counter can never be shadowed by an external one. \texttt{@fw:x} is always this
document's item, \texttt{@fwrev:fw:x} is always the other document's.

\section{An external citation is text, not a link}\label{an-external-citation-is-text-not-a-link}

The target lives in a different PDF, so TeXSmith emits the formatted reference
as plain text rather than a \texttt{\textbackslash{}hyperref} that would resolve to nothing. Local
references keep their live link.

\section{Diagnostics}\label{diagnostics}

This is the part that earns the feature. All of these are Python warnings, shown
by default and promotable to hard failures with \texttt{PYTHONWARNINGS=error}:
...
\midrule
the key is not published by the declared inventory & warning, and the citation renders as \texttt{[?fwrev:fw:disparu]} \\
the inventory file does not exist yet & warning, citations render as \texttt{[?…]} \\
...
\begin{code}{text}{}{}
\begin{Verbatim}[commandchars=\\\{\},breaklines, breakanywhere, commandchars=\\\{\}]
review/hardware\PYZhy{}review.md:0: UserWarning: Cross\PYZhy{}reference \PYZsq{}@fwrev:fw:disparu\PYZsq{} is not published by …
\end{Verbatim}
\end{code}
...
\section{Build order}\label{build-order}

\texttt{hardware-\allowbreak{}review.pdf} now depends on \texttt{firmware-\allowbreak{}review.refs.json}, which is a
Makefile edge:
...
\texttt{.aux}: a missing inventory is a warning and not a failure, so the first build
produces the inventories and the second resolves the citations.

\section{Not implemented yet}\label{not-implemented-yet}

Links from one PDF into another (\texttt{xr-\allowbreak{}hyper} does this with a PDF-viewer \texttt{GoToR}
