...
/ Internal References: Links that point to other sections within the same document or to other documents within the same project.
/ External References: Links that point to resources outside of the current project, such as websites or external documents.
/ Bibliographic References: Citations that refer to external publications, articles, or books. They are formatted using a citation style (APA, MLA, …) and gather into a bibliography.
/ Footnotes: Additional information parked at the bottom of the page, marked with a superscript number.
/ Equations, Tables, Figures, Listings: Numbered floats with an anchor, referenced by their key.
/ Tags/Index: Keywords associated with a section or topic, gathered into a printed index.
...
A heading with no `{#id}` still has an _implicit_ id — GitHub's slug of its
...
the heading an explicit `{#sec:…}`. See Headings for the slug
...
Printed output uses the usual #ts-logo("LaTeX") link commands:
...
#ts-callout(kind: "note", title: [A key that starts with a digit needs the brackets])[
A bare `@key` must begin with a letter, so the keys Zotero and Better
#ts-logo("BibTeX") generate (`1RgTv`, `7HA7H`) are only reachable bracketed:
`@[1RgTv]`. The printer writes such a key that way too, never bare. An
all-digit label stays a footnote.]

#ts-callout(kind: "note", title: [The footnote spelling of a citation is deprecated])[
...
The #ts-logo("LaTeX")-flavoured compatibility forms keep working: `\begin{equation}
...
The id is emitted verbatim as the #ts-logo("LaTeX") `\label`. Both web and print outputs
