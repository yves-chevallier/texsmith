...
/ Internal References: These are links that point to other sections within the same document or to other documents within the same project.
/ External References: These are links that point to resources outside of the current project, such as websites or external documents.
/ Bibliographic References: These are citations that refer to external publications, articles, or books. They are often formatted using a specific citation style (e.g., APA, MLA) and may include a bibliography section at the end of the document.
/ Footnotes: Footnotes provide additional information or citations without cluttering the main text. They are typically indicated by a superscript number in the text, with the corresponding footnote text provided at the bottom of the page or section.
/ Equations: Mathematical equations can be included in the document using LaTeX syntax. Equations can be labeled and referenced throughout the text.
/ Tables: Tables are used to present data in a structured format with rows and columns. They can be labeled and referenced within the document.
/ Figures: Figures are images, charts, or diagrams included in the document. They can be labeled and referenced throughout the text.
/ Listings/Code Blocks: Code blocks are used to display code snippets in various programming languages. They can be labeled and referenced within the document.
/ Tags/Index: Tags or index entries allow you to associate keywords with specific sections or topics in the document, making it easier to locate related information.
...
Markdown lacks native bibliography support, so TeXSmith reuses the footnote syntax and BibTeX/front matter keys. See the documentation on #link("../guide/features/bibliography.md")[Bibliography management] for more details.
