...
/ `#id`: An anchor. `@id` then refers to it, and the host decides which counter the
anchor joins: a heading is a section, a `Table:` line is a table, an image
is a figure.
/ `lang=`: The language of the element, for hyphenation, quotes and typographic
spacing. The document default is the root `lang:` key.
/ `media=`: Where the element is rendered: `all` (default), `print` or `web`. A word, a
