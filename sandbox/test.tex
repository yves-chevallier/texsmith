\documentclass{article}
\usepackage{fontspec}
\usepackage{parskip}
\setmainfont{Latin Modern Roman}


\usepackage{header}
\usepackage[unicode, hidelinks]{hyperref}

\begin{document}

\tableofcontents

\section{Language Support}\label{language-support}

TeXSmith can render documents in multiple languages and scripts, including those with complex typesetting requirements. Below are examples of various
languages and scripts supported by TeXSmith, showcasing its versatility in
handling diverse linguistic content.

\subsection{Japanese ({\japanesefont 日本語})}\label{japanese}

Spoken by over 125 million people, Japanese is the official language of Japan.

月日は百代の過客にして、行きかふ年も旅人なり。

舟に乗り、馬に乗りて、初秋の風に吹かれつつ、

道のべの小草を分け、山川を越えて、

心にかかる景色を求めて歩みゆく。

\subsection{Korean ({\koreanfont 한국어})}\label{korean}

Spoken by about 80 million people, Korean is the official language of both South Korea and North Korea.

이 몸이 죽고 죽어 일백 번 고쳐 죽어

백골이 진토되어 넋이라도 있고 없고

임 향한 일편단심이야 가실 줄이 있으랴

어찌 한 마음 변치 않으리오.

\subsection{Syriac ({\syriacFullfont ܠܫܢܐ ܣܘܪܝܝܐ})}\label{syriac}

Old language of Edessa (modern-day Şanlıurfa, Turkey), used in early Christian texts.

ܛܘܒܝܗܘܢ ܠܡܣܟܢ̈ܐ ܒܪܘܚܐ، ܕܕܝܠܗܘܢ ܗܝ ܡܠܟܘܬܐ ܕܫܡܝ̈ܐ. ܛܘܒܝܗܘܢ ܠܐܒܝ̈ܠܐ، ܕܗܢܘܢ ܢܬܒܝܐܘܢ. ܛܘܒܝܗܘܢ ܠܡܟܝ̈ܟܐ، ܕܗܢܘܢ ܢܪܬܘܢ ܠܐܪܥܐ. ܛܘܒܝܗܘܢ ܠܐܝܠܝܢ ܕܟܦܢܝܢ ܘܨܗܝܢ ܠܟܐܢܘܬܐ، ܕܗܢܘܢ ܢܣܒܥܘܢ.

\subsection{Greek ({\greekfont ελληνικά})}\label{greek}

From Homer \emph{Odyssey} (8\textsuperscript{th} century BC)

Άντρα, Μούσα, πες μου τον πολυμήχανο, που πάρα πολλά
ταλαιπωρήθηκε, αφού κατέστρεψε την ιερή πολιτεία της Τροίας·

και είδε τις πόλεις πολλών ανθρώπων και γνώρισε τον τρόπο τους,
και πολλές λύπες έπαθε στη θάλασσα μέσα στην καρδιά του,

προσπαθώντας να σώσει και τη δική του ζωή
και την επιστροφή των συντρόφων του.



\subsection{Chinese Trad. ({\chinesefont 繁體中文})}\label{chinese-trad}

From Tao Yuanming (365--427), 歸園田居

少無適俗韻，性本愛丘山。

誤落塵網中，一去三十年。

羈鳥戀舊林，池魚思故淵。

開荒南野際，守拙歸園田。

久在樊籠裡，復得返自然。

\subsection{Arabic}\label{arabic}

Arabic is spoken by over 400 million people worldwide and is the liturgical language of Islam.

قِفا نَبْكِ مِنْ ذِكرَى حبيبٍ ومَنزِلِ

بِسِقطِ اللِّوَى بَيْنَ الدَّخول فَحَوْمَلِ

فَتُوضِحَ فَالمِقراةِ لَم يَعفُ رَسمُها

لِما نَسَجَتها مِن جَنُوبٍ وشَمألِ

تَرَى بَعَرَ الأرْآمِ في عَرَصاتِها

وَقِيْعانِها كَأنَّهُ حَبُّ فُلْفُلِ

\subsection{Devanagari (देवनागरी)}\label{devanagari}

Language of Sanskrit, Hindi, Marathi, and others; used by over 600 million people.

न जायते म्रियते वा कदाचिन्

नायं भूत्वा भविता वा न भूयः।

अजो नित्यः शाश्वतोऽयं पुराणो

न हन्यते हन्यमाने शरीरे॥

\subsection{Urdu (اردو)}\label{urdu}

Urdu is spoken by over 170 million people, primarily in Pakistan and India.

عشق ایک مِیرُی آفت ہے

دل کو لے کر ڈوب ہی جاتی ہے

درد کی لہر اٹھتی جائے

اشک بھی ساتھ بہہ ہی جاتے ہیں

\subsection{Sinhala (සිංහල)}\label{sinhala}

Singhala is spoken by about 17 million people, primarily in Sri Lanka.

සඳ එළිය පුරා රැය නිල් සිලුවෙන් හැඬේලා

සඳහනෙකු මෙන් සිත් තුළ හඬක් වී පාවෙයි।

සිහිනෙන් පාරවල් හරහා ලංවීය මල් සුවඳක්

සරත් රැයේ සුවිසෙලිනි මනමේ තනුවේ රැදී.

\subsection{Khmer (ខ្មែរ)}\label{khmer}

Classical Khmer poetic style (Chbab form). Khmer is spoken by over 16 million people, mainly in Cambodia.

យប់ស្ថប់ត្រង់ ព្រៃស្រពន់សូរ ស្ទឹងស្ទូងតាមស្រមោលដើមឈើ

ខ្យល់អូលីល មកពីភ្នំឆ្ងាយ ប៉ះមកដល់ចិត្តអើយឆ្ងាយណា

សូរសត្វបក្សី លេបលាន់តាមសែន ក្លិនផ្កាឈូករាត្រីរំលេច។

\subsection{Telugu (తెలుగు)}\label{telugu}

From Nannaya (11\textsuperscript{th} century), \emph{Andhra Mahābhāratam}. Teluga is spoken by over 80 million people primarily in India.

నన్నయ భట్టుని వాక్కులు నదీవేగమువలె ప్రవహించి

పాండవ కథామృతము లోకమును నింపెను।

ధర్మమునకు మార్గదర్శి పాండవుని గాథలతో

తెలుగు భాషకు తొలిమకుటం కీర్తి ప్రసరించెనె.

\subsection{Georgian (ქართული)}\label{georgian}

Georgian is the official language of Georgia and the main language of Kartvelian
speakers. An estimated 4--5 million people speak Georgian worldwide.

ქართული ენა კავკასიონის მთებს შუ에서 დაბადებული ძველი ხმავია

ქრონიკებში ხელნაწერებში და ხალხურ სიმღერებში ეს ენა ხალხის ისტორიას მოჰყვება

თბილისის ქუჩის საუბრიდან სოფლის სუფრის ტოსტებამდე

ქართული სიტყვები მეგობრობისა და სტუმართმოყვარეობის სულს ატარებს

\subsection{Armenian (Հայերեն)}\label{armenian}

Armenian is the official language of Armenia and a central language for the
Armenian diaspora. There are roughly 6--7 million speakers worldwide.

Հայերենը միայն պետական լեզու չէ՝ այլ նաեւ հոգեւոր հիշողության տանող ուղի է։

Դարերի ընթացքում գրված տարեգրությունները՝ սաղմոսները եւ ժողովրդական երգերը՝

այս լեզվով են պահպանել ժողովրդի ուրախությունն ու ցավը։

\subsection{Russian ({\cyrillicsfont Русский})}\label{russian}

Russian is an East Slavic language with over 258 million speakers worldwide and is an official language in Russia, Belarus, Kazakhstan, and Kyrgyzstan.

Мой дядя самых честных правил,
когда не в шутку занемог,
он уважать себя заставил
и лучше выдумать не мог.

\subsection{Hebrew ({\hebrewfont עברית})}\label{hebrew}

Hebrew is a Semitic language spoken by over 9 million people worldwide and is the official language of Israel.

הַלֵּילָה אֲנִי חוֹלֵם עַל שָׂפוֹת הָעֵת בָּאוּ בַּחוֹף
קוֹל הַגַּלִּים מְשַׁחֵק עִם הָרֶגַע
וְהַלֵּב כּוֹסֵף לְבֵית כָּל שִׁירַי.

\subsection{Thai ({\thaifont ไทย})}\label{thai}

Thai is the official language of Thailand and is spoken by more than 60 million people.

สายลมพัดผ่านทุ่งข้าวเขียวขจี
กลิ่นดอกไม้โชยมากับเสียงระฆังวัด
หัวใจคิดถึงรอยยิ้มและคำทักทายอบอุ่นของผู้คน.

\subsection{Bengali ({\bengalifont বাংলা})}\label{bengali}

Bengali (Bangla) is an Indo-Aryan language with over 250 million speakers in Bangladesh, India, and beyond.

এই নদী মাঠ আর গাছের ছায়া
মানুষের হাসি আর শোকের গান
সব মিলিয়ে বাংলা ভাষা হৃদয়ে রাখে প্রাণের গল্প।

\subsection{Tamil ({\tamilfont தமிழ்})}\label{tamil}

Tamil is a Dravidian language spoken by more than 80 million people in India, Sri Lanka, and the diaspora.

காற்றின் மணம் நனைந்து வரும்
பாட்டின் சொல் மனதில் நிற்கும்
தமிழ் மண்ணின் நெஞ்சில் வாழும் மரபின் இனிமை.

\subsection{Tibetan ({\tibetanfont བོད་ཡིག})}\label{tibetan}

Tibetan is spoken by millions across Tibet and the Himalayan region, carrying a rich literary and spiritual tradition.

བོད་ཡིག་གི་ཚིག་སྨོན་རྒྱལ་ཁབ་གི་སྙིང་ནས་བྱུང་
རླུང་དང་སྣུམ་གྱི་སྒྲ་དང་མཉམ་དུ་སྐད་ཆ་སྣང་བ་བསྐུར།
ལམ་སེང་གི་སྒྲོན་མ་བཞུགས་སྒོམ་དང་མཉམ་བཞག་པའི་ཡིད་ཀྱི་གདུང་འདུག།

\subsection{Amharic (አማርኛ)}\label{amharic}

Amharic is a Semitic language spoken mainly in Ethiopia, where it is the official
working language. There are nearly 60 million speakers in total, counting first
and second language use.

GRAS: \textbf{ማርኛ በኢትዮጵያ የመንግሥት}

አማርኛ በኢትዮጵያ የመንግሥት ቋንቋ እና የብዙ ብሔሮች መገናኛ መንገድ ናት።

መዝሙሮች፣ ታሪካዊ ዘገባዎች እና የዕለት ተዕለት ተወላጅ ቃላት ውስጥ ይህች ቋንቋ የሕይወትን ድምጽ ታትማለች።

ከከተማ መንገዶች ጀምሮ እስከ ገጠር ገበያዎች ድረስ፣ አማርኛ የሰዎችን ልምድ ታስተካክላለች።

\subsection{Burmese (မြန်မာ)}\label{burmese}

Burmese (Myanmar language) is the official language of Myanmar and the mother
tongue of the country's largest ethnic group. About 33 million native speakers
and over 40 million total speakers use it.

မြန်မာဘာသာကို ရှေးအခေတ်ကပင် ဗိမာန်၊ ရာဇဝင်စာအုပ်တွေထဲက ဗျာဒိတ်စာနဲ့အတူ ဆက်ခံလာကြတယ်။

ဒိုင်းလယ်ရွာက ညအချိန်ထဲက ပန်းချီသရုပ်ဆောင် မှောင်မိုက်ပြဇာတ်ထဲမှာပါ ဒီဘာသာစကား ခံစားချက်ကိုဖော်ပြတတ်တယ်။

လမ်းဘေးဆိုင်က ဖျော်ဖြေမှု၊ ဘုရားကျောင်းရဲ့ ဂာသာတရားတွေ

အားလုံးမှ မြန်မာစကားဟာ လူမှုဘဝရဲ့ နှလုံးတံခါးကို ဖွင့်ပေးနေပါတယ်။

\end{document}
