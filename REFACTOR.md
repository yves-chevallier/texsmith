...
1. [x] **Collapse orchestration** — fait en 2 temps : (A) `_render_document`/`_build_runtime_common` lisent `document`/`request`/`generation` depuis le `context` au lieu de 11 kwargs threadés ; (B) `pipeline.py` (API batch + `ConversionBundle`/`LaTeXFragment`/`convert_documents`/`to_template_fragments`) fusionné dans `core.py` et supprimé — moteur de conversion désormais en un seul module. Importeurs repointés vers `conversion.core` ; API publique `texsmith.*` inchangée. *(Note : `convert_document` mono-doc conservé — fusionner dans `_render_document` créerait une fonction de ~250 L, anti-lecture.)*
2. [x] **Orchestration moteur LaTeX sortie de la CLI.** `render.py` réimplémentait inline (sélection binaires tectonic/biber/makeglossaries, build command/env, run) ce que `ConversionService.build_pdf` faisait déjà — méthode jusque-là **sans aucun appelant**. `render.py` délègue désormais à `_SERVICE.build_pdf(...)` (1225→1150 L) ; la CLI ne garde que présentation, copie PDF et fichier de deps. `build_pdf` gagne un paramètre injectable `run_engine` (couture de test propre) — le hook `render.run_engine_command` est conservé et passé en argument. Tests de sélection binaire repointés vers le module `service` (où vit désormais la sélection). *(Les hooks `render.shutil/subprocess` restent — utilisés par la copie PDF / deps qui demeurent dans la CLI.)*
3. [x] **Pipeline Typst sorti de la CLI.** Le module entier `ui/cli/commands/typst_emit.py` (560 L, 0 dépendance CLI — `render_typst_document`/`build_typst_pdf` sont de la logique bibliothèque pure) déplacé en `core/conversion/typst.py` (couche conversion, symétrique de `core.py` côté LaTeX). `render.py` importe désormais depuis la lib. `_citation_label` promu en `citation_label` (public, writer Typst). Les 3 privés `core.conversion.templates` (`_build_mustache_defaults`/`_replace_mustaches_in_html`/`_resolve_callout_style`) deviennent des imports **intra-paquet** (sibling de `typst.py`) — plus de franchissement de couche CLI→core. *(Non fait : faire passer le chemin Typst par `ConversionService.execute` — délibérément plus léger, hors scope.)*
4. [x] **Fuite IR — traitée/clôturée après enquête.**
   - `exercise` : **supprimé** entièrement (admonition custom non standard — `read_exercise`, `_render_exercise`, `_EXERCISE_NOISE`, état `solutions`/`add_solution`/`exercise_counter`/`next_exercise`).
   - `rich_table` : **pas une fuite** — le reader produit déjà un `ir.Table` typé (model + cells). Le `render_table_html(model)` + re-parse dans les writers LaTeX/Typst est un round-trip *interne* réutilisant le renderer de l'extension tables (SSOT), pas un passthrough reader→writer. Le dupliquer par backend coûterait ~870 L (rejeté par le contrat IR). Laissé tel quel.
   - `snippet` : passthrough HTML **délibéré et documenté** (payload d'extension opaque ; le parsing est filesystem-aware — résolution de chemins + digest — donc légitimement côté write, pas dans le reader pur). Le typer coupleraient reader↔filesystem/plugin. **De-leak partiel fait** : la glu BeautifulSoup + 4 imports privés sortie du writer vers un point d'entrée unique `snippet.render_snippet_latex(html, context)` ; `_render_snippet` du writer = 1 appel. Tout le traitement snippet-HTML vit désormais dans le plugin.
5. [x] **`DoiBibliographyFetcher` shim supprimé** (reporté du Lot 1). Le global mutable `DoiBibliographyFetcher` (cache + couture de test « for compatibility with monkeypatch ») est retiré. `_resolve_bibliography_fetcher()` devient la **factory unique** (import paresseux de la vraie classe, sans cache global) ; `_ensure_doi_support()` ne renvoie plus que `(lookup_error, normalise)`. Les tests patchent désormais la factory (`_resolve_bibliography_fetcher`) au lieu du global — couture à but unique, plus de shim.
6. [x] **Outillage fonts relocalisé.** ~780 L (acquisition CTAN `.sty`/stub + Plex/emoji + `_prepare_fallback_context`/`_prepare_mono_font` + helpers) sorties de `fragments/fonts/__init__.py` (951→**101 L**) vers `texsmith/fonts/provisioning.py` (883 L) ; le fragment importe ce qu'il consomme. Relocalisation **pure** (bloc déplacé intact, zéro changement de référence interne, zéro changement de comportement). Pas de cycle : `texsmith/fonts/` importe déjà `core.http`/`core.user_dir`. **Reste LaTeX-spécifique** (sortie `.sty`/luaotfload) — la séparation agnostique/LaTeX pour servir Typst serait un chantier distinct (#6bis, non demandé).
