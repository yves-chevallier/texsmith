...
4. **Fermer la fuite IR** : lower `snippet`/`rich_table` dans le reader (plus de `BeautifulSoup`/`HtmlReader` rappelés depuis les writers). [x] Rôle `exercise` **supprimé** entièrement (admonition custom non standard, non documentée — `read_exercise`, `_render_exercise`, `_EXERCISE_NOISE`, et l'état dédié `solutions`/`add_solution`/`exercise_counter`/`next_exercise`). Restent `snippet` (légitime) et `rich_table` à transformer en nœuds IR typés.
