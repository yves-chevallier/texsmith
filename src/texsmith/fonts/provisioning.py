"""Font provisioning for the ``ts-fonts`` fragment.

LaTeX font-package acquisition (CTAN ``.sty`` download / stub generation, Plex
and emoji font files) and the family / glyph-fallback / monospace context the
``ts-fonts`` fragment injects. Extracted from the fragment package so all font
tooling lives under :mod:`texsmith.fonts` rather than inside a fragment.
"""
