...
\BLOCK{- set ns = namespace(has_tabularx=False, has_marginnote=False, has_longtable=False) }
...
\BLOCK{- endif }
\BLOCK{- if name == "longtable" -}
\BLOCK{- set ns.has_longtable = True -}
\BLOCK{- endif }
...
\BLOCK{- endif }
\BLOCK{- if ns.has_longtable }
% ``longtable`` hard-codes ``: `` as its caption separator while babel-french
% patches ``\@makecaption`` to use ``\CaptionSeparator`` (`` – ``). Align
% ``\LT@makecaption`` on the same separator when one is installed so a
% document mixing floating tables and longtables keeps one caption style.
\makeatletter
\AtBeginDocument{%
  \ifdefined\CaptionSeparator
    \def\LT@makecaption#1#2#3{%
      \LT@mcol\LT@cols c{\hbox to\z@{\hss\parbox[t]\LTcapwidth{%
        \sbox\@tempboxa{#1{#2\CaptionSeparator}#3}%
        \ifdim\wd\@tempboxa>\hsize
          #1{#2\CaptionSeparator}#3%
        \else
