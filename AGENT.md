...
- **Readers** (`readers/`): `tmark.parse` for every Markdown source; the
  bs4-based `readers/html` for an `.html` input or a page whose front matter
  says `press.reader: html`. There is no `--reader` option.
- **IR** (`ir/model.py`): the generated Python mirror of tmark's committed
  schema — regenerated by `scripts/gen_ir_models.py`, excluded from ruff, and
  compared byte for byte in CI. Never hand-edit it.
...
- `scripts/gen_ir_models.py` regenerates `src/texsmith/ir/model.py` from tmark's
  schema; CI compares its output byte for byte.
