...
\Gloves\quad{} préparation \textbf{\VAR{time.preparation | default('--', true)}}~min \\
\Oven\quad{} cuisson \textbf{\VAR{time.cooking | default('--', true)}}~min \\
\rule{\linewidth}{1.5pt}
...
\BLOCK{ if ingredients }
  \BLOCK{ for ingredient in ingredients }
    \VAR{quantity(ingredient.quantity | default('', true))} & \VAR{ingredient.name | default(ingredient.step | default('', true), true)} &
    \BLOCK{ if loop.first }
      \VAR{note_text}
    \BLOCK{ endif }\\
  \BLOCK{ endfor }
