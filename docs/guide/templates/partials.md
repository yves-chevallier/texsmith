...
| Fragment | Macro / environment | Keys | Notes |
| --- | --- | --- | --- |
...
| | `\tslogo{XeLaTeX}` | — | a TeX logo word (`typography.tex-logos`) through `hologo`; `\TeX`, `\LaTeX`, `\LaTeXe` are the kernel's |
...
| `ts-equations` | — | — | a marker, not a package: the writer names it when an equation carries a label, so the Typst template knows to number equations. Nothing to define on the LaTeX side |
...
(`#id` → `id`, classes → `class={a,b}`, `key=val` as is); `lang` and `media`
never are.
...
template redefines a function after it, as a LaTeX template redefines a macro
...
1. **Restyle through the pgfkeys family.** `tscallout` and `tscode` are
...
2. **Redefine the macro, same signature.**
...
3. **Replace the fragment.** `press.fragments: {disable: [ts-code], append:
