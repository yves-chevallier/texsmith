...
TeXSmith 0.7 parses its Markdown with [TMark](https://github.com/yves-chevallier/tmark),
...
but most of them are now *deprecated sugar*: the parser warns, and the
...
`--fix` only applies the *safe* fixes — the mechanical one-to-one rewrites of
...
Every row is accepted today. "fmt" in the *Horizon* column means the spelling
...
| ------ | --------- | ------- |
| `[^key]` citation | `@key` | fmt; `[^1]` stays a *footnote*; meaning-preserving — both are the short, parenthetical citation, and `--fix` inserts a space when the `[^key]` hugs the word before it |
...
| `::: margin` | `::: aside` | fmt |
| `Table:` line *before* the table | `Table:` line *after* the table | indefinite (Pandoc accepts both) |
...
them, because in each case the *old* reading was the accident.

Lazy continuation lines
:   CommonMark continues a paragraph inside a block quote or a list item even
...
List indentation
:   A nested list item must be indented to the *content column* of its parent
...
`_` inside words
:   CommonMark never opens emphasis inside a word, so `snake_case_name` and
...
HTML blocks
:   CommonMark's seven HTML-block kinds decide where an HTML block ends. Raw
...
Entity handling
:   Named and numeric HTML entities (`&nbsp;`, `&#x2014;`) are decoded to the
...
Attribute lists need a host
:   `{#id .class}` attaches to the element *immediately* before it (or to the
...
`\@` and `\#` escapes
:   `@` before a word and `#` before `[` or `(` are now sigils. They are
...
See also [Diagnostics](diagnostics.md) for the diagnostic codes and how to
