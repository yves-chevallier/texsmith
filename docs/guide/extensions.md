...
## The MkDocs companion

The same document must render on a Material site. TeXSmith ships MkDocs plugins
for that, registered as entry points:

```yaml
plugins:
  - texsmith.index     # injects collected index tags into search_index.json
  - texsmith.counters  # renders counter items and their references on the site
```

When you use `mkdocs-texsmith`, the plugin appends what it needs automatically
unless you disable it with the `inject_markdown_extension` option. See
