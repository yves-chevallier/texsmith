...
*line* is the shorthand: the paragraph right after the float, spelled
...
The line attaches to the float *before* it when that float is of the matching
kind and has no caption yet, otherwise to the float *after* it — so the
